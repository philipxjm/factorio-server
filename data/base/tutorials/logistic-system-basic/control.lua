...
story_table = {
  {
    ...
    },
    {
      init = function()
        ...
        recreate_entities(loop_entities, {filter = {"player"}})
        camera.hold(global.camera, {time = 0.5})
      end,
      update = function()
        create_entities_on_tick()
        camera.update(global.camera)
        local character = surface().find_entities_filtered({name = "player"})[1]
    ...
    },
    {
      ...
      init = function()
        ...
        player.set_controller{type = defines.controllers.character, character = surface.create_entity{force = "player", name = "player", position = {0, 0}}}
        ...
        set_info()
        player.get_quickbar().insert{name = "roboport", count = 1}
        player.get_quickbar().insert{name = "medium-electric-pole", count = 10}
        player.insert"steel-axe"
    ...
    },
    {
      ...
      condition = function()
        ...
        if character and character.valid then
          for k, name in pairs (junk) do
            if character.get_inventory(defines.inventory.player_trash).get_item_count(name) > 0 then
              return true
            end
          end
        end
    ...
    intermission,
    {
      ...
      update = function()
        ...
        if global.robot.get_item_count("stone") > 0 then
          ...
          global.robot_tag.color = {r = 0.2, g = 1, b = 0.2}
        elseif player().get_inventory(defines.inventory.player_trash).is_empty() then
