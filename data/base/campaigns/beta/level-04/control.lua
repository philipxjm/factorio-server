...
script.on_init(function()
  ...
  game.forces.enemy.evolution_factor = 0.6
  ...
  technology_list["follower-robot-count-4"].researched = true
...
{
  {
    ...
    },
    {
      condition = story_elapsed_check(5),
      action = function()
        set_goal({"goal-research-oil-processing"})
      end
    },
    {
      condition =
    ...
    },
    {
      ...
      function()
        ...
        set_goal({"goal-build-small-plane"})
      end
    },
    {
      condition = story_elapsed_check(5),
...
init_attack_data = function()
  ...
  game.forces.enemy.evolution_factor = global.attack_data.initialevolution
end
...
function handle_attacks()
  ...
  if game.players[1].character and global.attack_data.until_next_attack <= 0 then
    game.forces.enemy.evolution_factor = game.forces.enemy.evolution_factor + global.attack_data.evolutionincrease
    if game.forces.enemy.evolution_factor > 0.85 then --don't allow behemoths in this campaign
      game.forces.enemy.evolution_factor = 0.85
    end
    local unitssent = game.players[1].surface.set_multi_command
    {
      command =
      {
